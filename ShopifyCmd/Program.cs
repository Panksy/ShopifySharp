...
namespace ShopifyCmd
{
    class Program
    {
        const string _commandNamespace = "ShopifyCmd.ShopifyCommands";
        static Dictionary<string, Dictionary<string, IEnumerable<ParameterInfo>>> _commandLibraries;

        static void Main(string[] args)
        {
            // Can take shopURL and accesstoken as parameters
            if (args.Length == 2)
            {
                AppSettings.MyShopifyUrl = args[0];
                AppSettings.AccessToken = args[1];

            }

            Console.Title = AppSettings.MyShopifyUrl;
        ...
        static string Execute(ConsoleCommand command)
        {
            ...
            if (command.Name.ToLower().Contains("delete"))
            {
                var input = ReadFromConsole(command.Name + "? Are you sure? Type Y or N, then Enter > ");
                if (input.ToLower() == "n")
        ...
        private static void ShowHelp(ConsoleCommand command)
        {
            Console.WriteLine("Default commands");
            Console.WriteLine("================");
        ...
        const string _readPrompt = "Shopify> ";
